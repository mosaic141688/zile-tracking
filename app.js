...
io.on("connection",function(socket){
  ...
  socket.on("dev-status",(cd)=>{
    db.setDeviceOnline(cd,()=>console.log("Device ${cd} Went offlne"))
  ...
  socket.on("disconnect",()=>{
    ...
    db.setDeviceOffline(cd,()=>console.log('device'+cd+'went offline'));
  })
})
...
app.post("/login",(req,resp)=>db.signinSchool(req.body.email,req.body.password,(res)=>resp.send(res)));
